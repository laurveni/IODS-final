...
where a and b are the coefficients, c is the intercept and e is the error term or noise, an unobservable random variable. The goal is to estimate the parameters a, b and the intercept c. The assumptions of the method are the following:
...
The p-values of both explanatory variables are very low (much less than 0.05) so they have a statistically significant relation with the Gross National Income per capita. We can thus reject the null hypothesis that the coefficients equal zero. However, the multiple R-squared of the model is 43.7%, which is quite low, and the standard error is pretty high. This means that the model explains little of the variablity so it would not produce precise predictions. 

Let us now look at Residuals vs Fitted, Normal Q-Q and Residuals vs Leverage plots to investigate if the hypotheses of the model are reasonable.
